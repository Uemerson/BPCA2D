...
class BPCA2D(nn.Module):
    ...
    def forward(self, x):
        ...
        unfold = F.unfold(x, kernel_size=K, stride=self.stride)
        patches = unfold.view(B, C, K * K, L).permute(0, 1, 3, 2)

        output = torch.zeros(B, C, H_out, W_out, device=x.device)

        # PCA per image
        for b in range(B):
            x_flat = patches[b].reshape(C * L, K * K)  # [C*L, K*K]

            x_mean = x_flat.mean(dim=0, keepdim=True)
            x_centered = x_flat - x_mean  # [C*L, K*K]

            cov = x_centered.T @ x_centered  # [K*K, K*K]
            _, eigvecs = torch.linalg.eigh(cov)  # ascending, [K*K, K*K]
            V = eigvecs[:, -1:]  # [K*K, 1] — largest eigenvector

            projected = x_centered @ V  # [C*L, 1]
            output[b] = projected.view(C, H_out, W_out)
