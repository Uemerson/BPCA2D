...
class BPCA2D(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = unfold.view(B, C, K * K, -1).permute(
            0, 1, 3, 2
        )  # [B, C, L, K*K]
        ...
        # PCA per image: each image has its own V
        for b in range(B):
            # patches[b] -> [C, L, K*K], colapsar C e L
            x_flat = patches[b].reshape(-1, K * K)  # [C*L, K*K]

            x_mean = x_flat.mean(dim=0, keepdim=True)
            x_centered = x_flat - x_mean
            _, _, V = torch.pca_lowrank(x_centered, q=1)  # V: [K*K, 1]
